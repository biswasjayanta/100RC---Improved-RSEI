...
library(dplyr)
library(ggplot2)
df <- read.delim("/Users/jayantabiswas/Library/CloudStorage/OneDrive-TheUniversityofMemphis/Khulna University/Modified_RSEI/Final output/coastal_long_cht_sur.txt", header=TRUE, sep = "\t")
...
ggplot(df, aes(x=Year, y=RSEI, color=Group, group=Group)) +
  stat_summary(fun=mean, geom="line", linewidth=1) +
  stat_summary(fun=mean, geom="point", size=3) +
  theme_minimal() +
  labs(title="Surat vs Chattogram Interaction plot: RSEI over Years by Group")
...
ggplot(df, aes(x = Year, y = RSEI,
  ...
  labs(
    title = "RSEI over Time: 100RC vs non-100RC (Surat vs Chattogram)",
    x = "Year",
...
ggsave("RSEI_interaction_Surat_vs_Chattogram.png", width = 6.5, height = 5, dpi = 600)
